...
)
{
	...
	if (u32DataType == HI_NET_DEV_AV_DATA) /*音视频数据*/
	{
		HI_S_AVFrame* pstruAV = (HI_S_AVFrame*)pu8Buffer;/*约定的帧类型数据*/

		if (pstruAV->u32AVFrameFlag == HI_NET_DEV_VIDEO_FRAME_FLAG) /*海思视频帧*/
		{
			//强制要求第一帧为I关键帧
			if (pThis->GetForceIFrameFlag()) /*如果已经设置为强制*/
			{
				if (pstruAV->u32VFrameType == HI_NET_DEV_VIDEO_FRAME_I) /*当前帧为I帧*/
					pThis->SetForceIFrameFlag(false);  /*取消设置为强制*/
				else
					return HI_SUCCESS;  /*当前帧不是I帧，不符合要求为I帧的条件，所以不处理*/
			}
	...
	{
		{
			...
			if (NULL == fPusherHandle)
			{
				...
				{
					mediainfo.u32VideoFps = 25; /*没配置，默认25帧*/
				}
				/*确认音频的编码参数？*/
				HI_S_Audio sAudio;
